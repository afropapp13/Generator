...
#include "Physics/BoostedDarkMatter/EventGen/DMELEventGenerator.h"
...
DMELEventGenerator::DMELEventGenerator() :
    KineGeneratorWithCache("genie::DMELEventGenerator")
{

}
//___________________________________________________________________________
DMELEventGenerator::DMELEventGenerator(string config) :
    KineGeneratorWithCache("genie::DMELEventGenerator", config)
{
...
void DMELEventGenerator::ProcessEventRecord(GHepRecord * evrec) const
{
    ...
    if(interaction->InitState().Tgt().IsNucleus()) {
        ...
        if(! evrec->HitNucleon()) {
            LOG("DMELEvent", pFATAL) << "No hit nucleon was set";
            gAbortingInErr = true;
            exit(1);
        }
    } // is nuclear target
    ...
    while (1) {
        ...
        iter++;
        LOG("DMELEvent", pINFO) << "Attempt #: " << iter;
	  ...
	  //          fNuclModel->GenerateNucleon(*tgt, hitNucPos);
        ...
        // Pick a direction
        // NOTE: In the kPSDMELEvGen phase space used by this generator,
        ...
        if(accept) {
            ...
            LOG("DMELEvent", pINFO) << "*Selected* Q^2 = " << gQ2 << " GeV^2";
            ...
            // For DMEL/Charm events it is set to be equal to the on-shell mass of
            ...
            LOG("DMELEvent", pNOTICE) << "Selected: W = "<< gW;
            ...
            evrec->SetDiffXSec(xsec, kPSDMELEvGen);
...
void DMELEventGenerator::AddTargetNucleusRemnant(GHepRecord * evrec) const
{
    ...
    LOG("DMELEvent", pINFO) << "Adding final state nucleus";
    ...
    LOG("DMELEvent", pINFO) << "Done";
    LOG("DMELEvent", pINFO) << *evrec;
}
//___________________________________________________________________________
...
void DMELEventGenerator::LoadConfig(void)
{
    ...
    fHitNucleonBindingMode = genie::utils::StringToDMELBindingMode( binding_mode );
...
double DMELEventGenerator::ComputeMaxXSec(const Interaction * in) const
{
    ...
    LOG("DMELEvent", pINFO) << "Computing maximum cross section to throw against";
...

    {
        ...
        for (int ilayer = 0 ; ilayer < max_n_layers ; ilayer++) {
          ...
          for (int itheta = 0; itheta < N_theta; itheta++){
              ...
              for (int iphi = 0; iphi < N_phi; iphi++) { // Scan around phi
                  ...
                  double xs = genie::utils::ComputeFullDMELPXSec(interaction,
        ...
        if (this_nuc_xsec_max > xsec_max) {
            ...
            LOG("DMELEvent", pINFO) << "best estimate for xsec_max = " << xsec_max;
        }
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
    SLOG("DMELEvent", pDEBUG) << interaction->AsString();
    SLOG("DMELEvent", pDEBUG) << "Max xsec in phase space = " << max_xsec;
    SLOG("DMELEvent", pDEBUG) << "Computed using alg = " << *fXSecModel;
#endif

    LOG("DMELEvent", pINFO) << "Computed maximum cross section to throw against - value is " << xsec_max;
    return xsec_max;
}
